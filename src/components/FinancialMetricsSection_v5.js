...
import { getAsOfDate, getDealProcessingSpeed, isDealPending } from '../data/dealDatesByUseCase';
...
function FinancialMetricsSection({ useCaseKey }) {
  ...
  const processingDays = getDealProcessingSpeed(useCaseKey);
  const isPending = isDealPending(useCaseKey);
  ...
  return (
    <section className="financial-metrics">
      ...
      <p className="timestamp-note">
        As of: {asOfDate}
        {processingDays && (
          <span className="processing-speed"> • Closed {processingDays} days after announcement</span>
        )}
        {isPending && (
          <span className="pending-status"> • Transaction pending</span>
        )}
      </p>
      <p className="caption">
