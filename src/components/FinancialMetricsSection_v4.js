  ...
  return (
    <section className="financial-metrics">
      ...
      <p className="timestamp-note">
        As of: {dealContext.reportingPeriod || 'Q1 2025'}
      </p>
      <p className="caption">
      ...
      <div className="metrics-grid">
        {["Valuation", "Returns", "Leverage & Risk"].map(category => groupedMetrics[category] && (
            <div key={category} className="metric-group">
              ...
              {/*  If category is Valuation, render comparison lines */}
              {/* Removed extra comparison rows since they're now embedded inline */}

              {/*  {category === "Valuation" && (*/}
              {/*      <>*/}
              {/*        {metrics.ebitdaMultiple && renderIndustryComparison("ebitdaMultiple")}*/}
              {/*        {metrics.ebitdaMultiple && renderIndustryComparison("revenueMultiple")}*/}
              {/*        {metrics.ebitdaMultiple && renderIndustryComparison("bookValueMultiple")}*/}
              {/*      </>*/}
              {/*  )}*/}
              </ul>
            </div>
