...
# Import overlap detection functions
from intent_overlap_blueprint import check_rule_conflicts, analyze_workspace_conflicts
...
def classify_intent(prompt: str, workspace_id: int = 0) -> Tuple[str, float, Dict]:
    """
    Classify intent using database-stored keyword/pattern matching rules with overlap detection
    Returns (intent, confidence_score, classification_details)
    """
        ...
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                ...
                if not rules:
                    ...
                    classification_details['error'] = 'No classification rules found'
                    return 'other', 0.3, classification_details
    ...
    for intent, config in patterns.items():
        ...
        if score > 0:
            ...
            matched_intents_details.append({
                'intent': intent,
                'raw_score': score,
    ...
    classification_details['all_scores'] = scores
    classification_details['matched_intents'] = matched_intents_details

    # Check for overlap/ambiguity - this is the key improvement
    if len(scores) > 1:
        sorted_scores = sorted(scores.items(), key=lambda x: x[1], reverse=True)
        top_score = sorted_scores[0][1]
        second_score = sorted_scores[1][1] if len(sorted_scores) > 1 else 0

        # If top two scores are very close, flag as overlap
        if second_score > 0 and (second_score / top_score) >= 0.7:  # Within 30% of each other
            classification_details['overlap_detected'] = True
            classification_details['overlap_warning'] = {
                'primary_intent': sorted_scores[0][0],
                'secondary_intent': sorted_scores[1][0],
                'primary_score': sorted_scores[0][1],
                'secondary_score': sorted_scores[1][1],
                'confidence_ratio': round(second_score / top_score, 2),
                'message': f"Ambiguous classification: '{sorted_scores[0][0]}' (score: {sorted_scores[0][1]:.1f}) vs '{sorted_scores[1][0]}' (score: {sorted_scores[1][1]:.1f})"
            }
    ...
    if scores:
        ...
        # Reduce confidence if overlap detected
        if classification_details['overlap_detected']:
            confidence *= 0.8  # Reduce confidence by 20% for ambiguous classifications

        return best_intent[0], confidence, classification_details
...
def log_interaction(workspace_id, user_id, thread_id, prompt, response, model,
    ...
    """Log an interaction to the interactions database with enhanced classification"""
    try:
        # If intent not provided, classify it with overlap detection
        if intent is None:
            intent, confidence, classification_details = classify_intent(prompt, workspace_id)

            # Add classification details to metadata
            if metadata is None:
                metadata = {}
            metadata['classification_confidence'] = confidence
            metadata['classification_method'] = 'keyword-based'
            metadata['classification_details'] = classification_details

            # Flag interactions with overlap issues
            if classification_details.get('overlap_detected'):
                metadata['classification_warning'] = 'overlap_detected'
                metadata['overlap_details'] = classification_details['overlap_warning']
        ...
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                ...
                print(f"Logged interaction {interaction_id} for user {user_id} with intent '{intent}'")

                # Log overlap warning if detected
                if metadata and metadata.get('classification_warning'):
                    print(f"  WARNING: {metadata['overlap_details']['message']}")
        ...
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                ...
                for row in cur.fetchall():
                    ...
                    # Add overlap warning flag if present
                    metadata = row['metadata'] or {}
                    if metadata.get('classification_warning'):
                        interaction_data['hasClassificationWarning'] = True
                        interaction_data['classificationWarning'] = metadata.get('overlap_details', {}).get('message')
    ...
    try:
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                ...
                # Intent performance with overlap analysis
                cur.execute("""
                    SELECT 
                        intent,
                        COUNT(*) as total_interactions,
                        ...
                        COUNT(CASE WHEN metadata->>'classification_warning' = 'overlap_detected' THEN 1 END) as overlap_warnings
                    FROM interactions 
                ...
                for row in cur.fetchall():
                    intent_stats.append({
                        ...
                        'overlapWarnings': row['overlap_warnings'],
                        'overlapRate': round((row['overlap_warnings'] / row['total_interactions']) * 100, 1)
                    })

                # Recent problems including overlap issues
                cur.execute("""
                    SELECT 
                        id,
                        prompt,
                    ...
                    WHERE workspace_id = %s AND user_id = %s 
                        AND (user_feedback IN ('bad', 'hallucination', 'irrelevant') 
                             OR metadata->>'classification_warning' = 'overlap_detected')
                        AND created_at >= %s
                    ORDER BY created_at DESC
                    LIMIT 10
                """, (workspace_id, user_id, since_date))
                ...
                for row in cur.fetchall():
                    ...
                    # Add overlap warning info if present
                    metadata = row['metadata'] or {}
                    if metadata.get('classification_warning') == 'overlap_detected':
                        problem_info['problemType'] = 'classification_overlap'
                        problem_info['overlapDetails'] = metadata.get('overlap_details', {})
                    else:
                        problem_info['problemType'] = 'user_feedback'

                    recent_problems.append(problem_info)
        ...
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    ...
                    FROM interactions 
                    WHERE workspace_id = %s AND user_id = %s 
                        AND metadata->>'classification_warning' = 'overlap_detected'
                """, (workspace_id, user_id))

                overlap_count = cur.fetchone()['overlap_count']

                return jsonify({
                    'totalInteractions': total,
                    'modelBreakdown': [{'model': row['model'], 'count': row['count']} for row in models],
                    'overlapWarnings': overlap_count,
                    'overlapRate': round((overlap_count / total) * 100, 1) if total > 0 else 0
                })
    ...
    try:
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("""
                    SELECT id, prompt, workspace_id FROM interactions 
                ...
                for row in interactions_to_classify:
                    ...
                    # Classify the intent with overlap detection
                    intent, confidence, classification_details = classify_intent(prompt, workspace_id)
                    ...
                    cur.execute("""
                        ...
                        WHERE id = %s
                    """, (intent, json.dumps(metadata), interaction_id))
                    ...
                    results.append({
                        ...
                        'overlap_detected': classification_details.get('overlap_detected', False)
                    })
                ...
                return jsonify({
                    ...
                    'overlap_rate': round((overlap_detected_count / classified_count) * 100,
                                          1) if classified_count > 0 else 0,
                    'method': 'keyword-based (Basic Tier) with overlap detection',
                    'results': results[:10],  # Show first 10 for review
                    'message': f'Classified {classified_count} interactions, {overlap_detected_count} had overlap warnings',
                    'recommendation': 'Review interactions with overlap warnings - they may indicate rule conflicts that need attention'
                })
...
def get_intent_stats():
    ...
    try:
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                # Overall intent distribution with overlap metrics
                cur.execute("""
                    SELECT 
                        intent,
                        COUNT(*) as count,
                ...
                for row in cur.fetchall():
                    count = row['count']
                    overlap_warnings = row['overlap_warnings']
                    total_classified += count
                    total_overlap_warnings += overlap_warnings
                    ...
                    intent_distribution.append({
                        ...
                        'overlapRate': round((overlap_warnings / count) * 100, 1) if count > 0 else 0
                    })
                ...
                # Service tier recommendation with overlap considerations
                service_tier = "Basic Tier (Keyword-based)"
                recommendation = None

                overlap_rate = (total_overlap_warnings / total_classified) * 100 if total_classified > 0 else 0

                if overlap_rate > 20:
                    recommendation = f"High overlap rate ({overlap_rate:.1f}%) detected. Consider reviewing intent rules for conflicts before upgrading service tiers."
                elif total_classified > 100:
                ...
                return jsonify({
                    ...
                    'overallAccuracy': round(
                        ...
                        1) if total_classified > 0 else 0,
                    'overallOverlapRate': round(overlap_rate, 1),
                    'totalOverlapWarnings': total_overlap_warnings
                })
...
def test_intent_classification():
    """Test classification on sample text with overlap detection"""
    data = request.get_json() or {}
    ...
    # Classify the test prompt with overlap detection
    intent, confidence, classification_details = classify_intent(test_prompt, workspace_id)
    ...
    try:
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    FROM interactions 
                    WHERE intent = %s AND workspace_id = %s
                    ORDER BY created_at DESC
                    LIMIT 3
                """, (intent, workspace_id))
        ...
        # Add overlap warning information if detected
        if classification_details.get('overlap_detected'):
            overlap_info = classification_details['overlap_warning']
            response_data['overlapDetected'] = True
            response_data['overlapWarning'] = {
                'message': overlap_info['message'],
                'primaryIntent': overlap_info['primary_intent'],
                'secondaryIntent': overlap_info['secondary_intent'],
                'confidenceRatio': overlap_info['confidence_ratio'],
                'recommendation': f"Consider reviewing rules for '{overlap_info['primary_intent']}' and '{overlap_info['secondary_intent']}' intents - they may have overlapping keywords that cause ambiguous classification."
            }
            response_data[
                'explanation'] += f" WARNING: Ambiguous classification detected between '{overlap_info['primary_intent']}' and '{overlap_info['secondary_intent']}'"

        # Add details about all matched intents
        if classification_details.get('matched_intents'):
            response_data['allMatchedIntents'] = classification_details['matched_intents']
        ...
        return jsonify({
            ...
            'explanation': f"Classified as '{intent}' based on keyword patterns with {round(confidence * 100, 1)}% confidence",
            'classificationDetails': classification_details,
            'overlapDetected': classification_details.get('overlap_detected', False)
        })


# Add new endpoint for overlap analysis
@interactions_api.route('/api/interactions/overlap-analysis/<int:workspace_id>/<user_id>', methods=['GET'])
def get_overlap_analysis(workspace_id, user_id):
    """Get detailed analysis of classification overlap issues"""
    days = request.args.get('days', 30, type=int)
    since_date = datetime.now() - timedelta(days=days)
    ...
    try:
        with get_interactions_db() as conn:
            with conn.cursor() as cur:
                # Get interactions with overlap warnings
                cur.execute("""
                    SELECT 
                        id,
                        prompt,
                ...
                for row in cur.fetchall():
                    metadata = row['metadata'] or {}
                    overlap_details = metadata.get('overlap_details', {})

                    primary_intent = overlap_details.get('primary_intent')
                    secondary_intent = overlap_details.get('secondary_intent')

                    if primary_intent and secondary_intent:
                        conflict_key = tuple(sorted([primary_intent, secondary_intent]))
                        if conflict_key not in intent_conflicts:
                            intent_conflicts[conflict_key] = {
                                'intent1': conflict_key[0],
                                'intent2': conflict_key[1],
                                'count': 0,
                                'examples': []
                            }
                        intent_conflicts[conflict_key]['count'] += 1
                        if len(intent_conflicts[conflict_key]['examples']) < 3:
                            intent_conflicts[conflict_key]['examples'].append({
                                'prompt': row['prompt'][:100] + '...' if len(row['prompt']) > 100 else row['prompt'],
                                'chosen_intent': row['intent'],
                                'confidence_ratio': overlap_details.get('confidence_ratio', 0)
                            })

                    overlap_cases.append({
                        'id': str(row['id']),
                        'prompt': row['prompt'][:150] + '...' if len(row['prompt']) > 150 else row['prompt'],
                        'chosenIntent': row['intent'],
                        'overlapDetails': overlap_details,
                        'timestamp': row['created_at'].isoformat() + 'Z'
                    })

                # Convert conflicts to list and sort by frequency
                frequent_conflicts = sorted(
                    intent_conflicts.values(),
                    key=lambda x: x['count'],
                    reverse=True
                )

                # Get workspace health from overlap blueprint if available
                workspace_health = None
                try:
                    from intent_overlap_blueprint import analyze_workspace_conflicts
                    conflict_analysis = analyze_workspace_conflicts(workspace_id)
                    workspace_health = {
                        'totalConflicts': conflict_analysis['summary']['total_conflicts'],
                        'highSeverity': conflict_analysis['summary']['high_severity'],
                        'moderateSeverity': conflict_analysis['summary']['moderate_severity']
                    }
                except Exception as e:
                    print(f"Could not get workspace health: {e}")
                ...
                return jsonify({
                    ...
                    'totalOverlapCases': len(overlap_cases),
                    'frequentConflicts': frequent_conflicts,
                    'workspaceHealth': workspace_health,
                    'recommendations': [
                                           f"Review rules for {conflict['intent1']} and {conflict['intent2']} - {conflict['count']} overlap cases"
                                           for conflict in frequent_conflicts[:3]
                                       ] + [
                                           "Consider adding more specific keywords to reduce ambiguity" if len(
                                               frequent_conflicts) > 0 else None,
                                           "Use the /api/intents/conflicts endpoint to get detailed conflict analysis" if workspace_health else None
                                       ]
                })
