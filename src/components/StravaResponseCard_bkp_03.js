...
const StravaResponseCard = ({ question, backendData, onSuggestionClick }) => {
  ...
  const handleSemanticElementClick = async (event, element) => {
    console.log('Semantic element clicked:', element);

    // Handle unit conversion for values
    if (element.type === 'value' && element.clickable_actions?.includes('convert_unit')) {
      // Determine target unit based on current unit
      let targetUnit;
      if (element.unit === 'miles') targetUnit = 'kilometers';
      else if (element.unit === 'kilometers') targetUnit = 'miles';
      else if (element.unit === 'seconds_per_mile') targetUnit = 'seconds_per_km';
      else if (element.unit === 'seconds_per_km') targetUnit = 'seconds_per_mile';

      if (targetUnit) {
        try {
          const response = await fetch('http://localhost:5002/api/strava/ai/convert-unit', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              value: element.value_raw || element.value,
              from_unit: element.unit,
              to_unit: targetUnit,
          ...
          if (result.success) {
            ...
            // TODO: Update the element in place
            alert(`Converted to: ${result.new_display}`);
          }
        } catch (error) {
    ...
    // For other actions, show the existing menu
    setSelectedToken({
      ...element,
      text: element.display,
      metadata: { clickable_actions: element.clickable_actions },
