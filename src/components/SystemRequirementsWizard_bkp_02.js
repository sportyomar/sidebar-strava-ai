...
const SystemRequirementsWizard = ({
  ...
  // Collapsed sections by default for viewport-friendly experience
  ...
  const handleExpandAll = () => {
    ...
    setCollapsedSections({
      projectSettings: !newExpandAll,
      organizationalSettings: !newExpandAll,
      marketSettings: !newExpandAll,
      technicalSettings: !newExpandAll
    });
  };
  ...
  const getSectionSummary = (category) => {
    const categoryFields = stepInfo[Object.keys(stepInfo).find(key => stepInfo[key].category === category)]?.fields || [];
    const selectedFields = categoryFields.filter(field => selections[field] !== null);
    const skippedFieldsList = categoryFields.filter(field => skippedFields[field]);

    if (selectedFields.length === 0 && skippedFieldsList.length === 0) {
      return 'Not configured';
    }

    const parts = [];
    if (selectedFields.length > 0) {
      parts.push(`${selectedFields.length} selected`);
    }
    if (skippedFieldsList.length > 0) {
      parts.push(`${skippedFieldsList.length} skipped`);
    }

    return parts.join(', ');
  };

  const renderCollapsedSection = (category, title) => (
    <div key={category} className={styles.collapsedSection}>
      <div
        className={styles.collapsedSectionHeader}
        onClick={() => toggleSectionCollapse(category)}
      >
        <div className={styles.collapsedSectionInfo}>
          <h3 className={styles.collapsedSectionTitle}>{title}</h3>
          <div className={styles.collapsedSectionSummary}>
            {getSectionSummary(category)}
          </div>
        </div>
  ...
  return (
    <div className={styles.container}>
      <div className={styles.card}>
        {/* Header - Unified with main infrastructure style */}
        <div className={styles.stepContent}>
          ...
          {/* Render either collapsed view or current step */}
          {!expandAll ? (
            <div className={styles.collapsedView}>
              {renderCollapsedSection('projectSettings', 'Project Settings')}
              {renderCollapsedSection('organizationalSettings', 'Organizational Settings')}
              {renderCollapsedSection('marketSettings', 'Market Settings')}
              {renderCollapsedSection('technicalSettings', 'Technical Settings')}
            </div>
          ) : (
            <>
              {/* Clickable Step Navigation - Only when expanded */}
              ...
              <div className={styles.expandedStepContent}>
                <h2 className={styles.expandedStepTitle}>
                  {stepInfo[currentStep].title}
                </h2>
                <p className={styles.expandedStepDescription}>
          ...
          {/* Enhanced Status Summary - Always visible */}
          {renderStatusSummary()}
        </div>
      </div>
    </div>
