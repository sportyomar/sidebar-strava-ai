...
const AccountPage = ({
  API_BASE_URL,
  fetchWithAuth,
  selectedWorkspace,     // ✅ From App.js
  setSelectedWorkspace,  // ✅ From App.js
  workspaces,           // ✅ From App.js
  onWorkspaceChange
}) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoadingTab(t => ({ ...t, organization: true }));
        const orgRes = await fetchWithAuth(`${API_BASE_URL}/api/account/organization`);

        if (!ignore) {
          if (orgRes.ok) setOrganization(await orgRes.json());
          setLoadedTab(l => ({ ...l, organization: true }));
        }
      } catch (e) {
        if (!ignore) setError('Failed to load account data.');
      } finally {
        if (!ignore) setLoadingTab(t => ({ ...t, organization: false }));
      }
    })();
    return () => { ignore = true; };
  }, []);
  ...
  const handleWorkspaceChange = useCallback(async (e) => {
    ...
    try {
      ...
      if (token) {
        await fetchWithAuth(`${API_BASE_URL}/api/user/last-workspace`, {
          method: 'PUT',
          headers: {
      ...
      // Then update local state (this will flow back to App.js)
      onWorkspaceChange(workspaceId);
      setSuccess('Workspace selection saved');
    } catch (error) {
      console.error('Failed to save workspace selection:', error);
      setError('Failed to save workspace selection');
    }
  }, [onWorkspaceChange, fetchWithAuth, API_BASE_URL]);
