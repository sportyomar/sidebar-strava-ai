...
function PlatformDocumentation({ user, isAuthenticated }) {
  ...
  useEffect(() => {
    ...
    // Fetch platform sections
    fetch('http://localhost:5002/api/platform-sections')
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) {
          setPlatformSections(data);
          console.log('Platform sections loaded:', data);
        } else {
          console.error('Invalid platform sections API response');
        }
      })
      .catch(err => {
        console.error('Failed to load platform sections:', err);
        setPlatformSections([]); // Set empty array on error
      });
  }, []);
  ...
  const handleCreatePlatformSection = async (sectionData) => {
    ...
    try {
      ...
      console.log('Creating platform section with data:', sectionData);
      ...
      const data = await response.json();
      console.log('Platform section API response:', data);

      if (response.ok) {
        // Refresh to show new platform section
        window.location.reload();
      } else {
        console.error('Platform section API error details:', data);
        throw new Error(data.error || data.message || 'Failed to create platform section');
      }
    } catch (error) {
      ...
      <div className={styles.layout}>
        ...
        <main className={styles.documentContainer} ref={documentContainerRef}>
          <div className={styles.document}>
            {!selectedDoc && !showCreationForm && !showPlatformSectionForm ? (
              <ExecutiveDashboard
                groupedDocs={groupedDocs}
              ...
              <DocumentCreationForm
                ...
                isAuthenticated={isAuthenticated}
              />
            ) : showPlatformSectionForm ? (
              <PlatformSectionCreationForm
                onCreateSection={handleCreatePlatformSection}
                onCancel={() => setShowPlatformSectionForm(false)}
                isCreating={isCreatingPlatformSection}
                user={user}
                isAuthenticated={isAuthenticated}
              />
            ) : (
