...
import styles from './BasicDataCanvas.module.css';
  ...
  const renderQueryResults = (group) => {
    ...
    if (dataTransform.lastQuery) {
      ...
      if (dataTransform.queryResults) {
        ...
        // Render data grid
        renderDataGrid(group, resultsX, resultsY + 140);
      }
    }
  };

  const renderDataGrid = (group, startX, startY) => {
    if (!dataTransform.queryResults || !dataTransform.queryResults.rows.length) return;

    const { rows, columns } = dataTransform.queryResults;
    const cellWidth = 120;
    const cellHeight = 30;
    ...
    group.append('text')
      ...
      .attr('font-size', 11)
      .attr('font-weight', '600')
      .text(`Preview (showing ${displayRows.length} of ${rows.length} rows):`);
    ...
    columns.forEach((col, colIndex) => {
      ...
      group.append('text')
        .attr('class', 'query-results')
        .attr('x', x + 5)
        .attr('y', gridStartY + headerHeight / 2)
        .attr('dominant-baseline', 'middle')
        .attr('fill', '#111827')
        .attr('font-size', 10)
        .attr('font-weight', '600')
        .text(col.length > 15 ? col.substring(0, 12) + '...' : col);
    });

    // Render data rows
    displayRows.forEach((row, rowIndex) => {
      const y = gridStartY + headerHeight + (rowIndex * cellHeight);
      ...
      columns.forEach((col, colIndex) => {
        ...
        group.append('text')
          ...
          .attr('fill', '#374151')
          .attr('font-size', 9)
          .text(displayValue.length > 15 ? displayValue.substring(0, 12) + '...' : displayValue);
      });
    });
    ...
    if (rows.length > 10) {
      group.append('text')
        .attr('class', 'query-results')
        .attr('x', startX)
        .attr('y', gridStartY + headerHeight + (displayRows.length * cellHeight) + 15)
        .attr('fill', '#6b7280')
        .attr('font-size', 9)
        .attr('font-style', 'italic')
        .text(`... ${rows.length - 10} more rows not shown`);
    }
  };
      ...
      <div className={styles.wrapper}>
        ...
        {dataTransform.connectionStatus !== 'disconnected' && (
          <div className={styles.connectionInfo}>
            <div className={styles.connectionInfoTitle}>
              Status: {dataTransform.connectionStatus}
            </div>
            {dataTransform.connectionInfo && (
              <div className={styles.connectionInfoText}>
