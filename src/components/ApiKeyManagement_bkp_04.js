...
const ApiKeyManagement = ({ selectedWorkspace, fetchWithAuth, API_BASE_URL, onError, onSuccess }) => {
  ...
  const [visibleProviderKeys, setVisibleProviderKeys] = useState(new Set());
  ...
  const loadProviderCredentials = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(`${API_BASE_URL}/api/account/workspaces/${selectedWorkspace.id}/provider-credentials`);
      if (res.ok) {
        const savedCredentials = await res.json();
        console.log('Loaded saved credentials:', savedCredentials);

        // Update credentials state with saved data
        Object.keys(savedCredentials).forEach(provider => {
          const saved = savedCredentials[provider];
          setCredentials(prev => ({
            ...prev,
            [provider]: {
              ...prev[provider],
              apiKey: '••••••••••••••••', // Show masked placeholder initially
              actualApiKey: saved.hasKey ? '[ENCRYPTED_KEY_SAVED]' : '', // Store reference to actual key
              endpoint: saved.endpoint || prev[provider].endpoint,
              apiVersion: saved.apiVersion || prev[provider].apiVersion,
              orgId: saved.orgId || prev[provider].orgId,
              deployments: saved.deployments || prev[provider].deployments,
              status: saved.status === 'unknown' ? 'connected' : saved.status // Treat unknown as connected
            }
          }));
        });
      }
  ...
  const toggleProviderKeyVisibility = useCallback((provider) => {
  ...
  const testConnection = async (provider) => {
    // If the key is masked (dots), we need to test the saved key on backend
    if (credentials[provider].apiKey === '••••••••••••••••') {
      // Test the saved key via backend endpoint
      try {
        const response = await fetchWithAuth(`${API_BASE_URL}/api/account/workspaces/${selectedWorkspace.id}/provider-credentials/${provider}/test`, {
          method: 'POST'
        });
        ...
        if (response.ok) {
          ...
          if (result.success) {
            ...
            onError(`${provider} connection failed: ${result.message}`);
          }
        }
      } catch (error) {
    ...
    // Original test logic for new keys
    setCredentials(prev => ({
      ...prev,
  ...
  const saveCredentials = async (provider) => {
    try {
      ...
      const response = await fetchWithAuth(`${API_BASE_URL}/api/account/workspaces/${selectedWorkspace.id}/provider-credentials/${provider}`, {
        method: 'POST',
        body: JSON.stringify({
    ...
    <div className={styles.spaceY6}>
      ...
      {activeSection === 'workspace' ? (
        ...
        <>
          ...
          {Object.keys(credentials).some(provider => credentials[provider].status === 'connected') && (
            <div className={styles.card}>
              <h5 className={styles.cardTitle} style={{ fontSize: '0.875rem', marginBottom: '1rem' }}>Active Connections</h5>
              <div className={styles.table}>
                <div className={styles.tableContainer}>
                  <table>
                    <thead className={styles.tableHeader}>
                      <tr>
                        <th className={styles.tableHeaderCell}>Provider</th>
                        <th className={styles.tableHeaderCell}>Status</th>
                        <th className={styles.tableHeaderCell}>Last Tested</th>
                        <th className={styles.tableHeaderCell}>Actions</th>
                      </tr>
                    </thead>
                    <tbody className={styles.tableBody}>
                      {Object.entries(credentials)
                        .filter(([_, cred]) => cred.status === 'connected')
                        .map(([provider, cred]) => (
                        <tr key={provider} className={styles.tableRow}>
                          <td className={styles.tableCell}>
                            <div className={styles.flexCenter} style={{ gap: '0.5rem' }}>
                              <div style={{
                                width: '1.5rem',
                                height: '1.5rem',
                                backgroundColor: provider === 'openai' ? '#10a37f' : provider === 'anthropic' ? '#d97706' : '#0078d4',
                                color: 'white',
                                ...
                                {provider === 'openai' ? 'AI' : provider === 'anthropic' ? 'C' : 'AZ'}
                              </div>
                              <span style={{ textTransform: 'capitalize' }}>
                                {provider === 'azureopenai' ? 'Azure OpenAI' : provider.charAt(0).toUpperCase() + provider.slice(1)}
                              </span>
                            </div>
                          </td>
                          <td className={styles.tableCell}>
                            <span className={`${styles.badge} ${getStatusClass(cred.status)}`}>
                              Connected
                            </span>
                          </td>
                          <td className={`${styles.tableCell} ${styles.textGray500}`}>
                            {cred.lastTested ? new Date(cred.lastTested).toLocaleDateString() : 'Not tested'}
                          </td>
                          <td className={styles.tableCell}>
                            <div className={styles.flexCenter} style={{ gap: '0.5rem' }}>
                              <button
                              ...
                              <button
                                onClick={() => {/* TODO: Delete credential */}}
                                className={`${styles.iconButton} ${styles.iconButtonDanger}`}
            ...
            <div className={styles.spaceY4}>
              <div className={styles.formGroup}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    type={visibleProviderKeys.has('openai') ? "text" : "password"}
                    className={styles.input}
                    placeholder="sk-..."
                  ...
                  <button
                    type="button"
                    onClick={() => toggleProviderKeyVisibility('openai')}
                    className={styles.iconButton}
                    style={{
                      position: 'absolute',
                      right: '8px',
                  ...
                  >
                    {visibleProviderKeys.has('openai') ?
